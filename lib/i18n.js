...
const xnoteI18n = (() => {
   ...
   * Concurrent calls are coalesced — only one init runs at a time.
   */
  ...
  async function loadMessages(lang) {
    // Only skip if cache has actual keys (empty object = previous fetch failed, retry)
    if (_messages[lang] && Object.keys(_messages[lang]).length > 0) return;
    try {
      // Try direct fetch first (works in extension pages like options.html)
      const url = chrome.runtime.getURL(`_locales/${lang}/messages.json`);
      const resp = await fetch(url);
      _messages[lang] = await resp.json();
    } catch (e) {
      // Direct fetch fails in content scripts (cross-origin). Ask background to load.
      try {
        const result = await chrome.runtime.sendMessage({
          action: 'xnote-load-locale',
          lang: lang,
        });
        if (result && result.data) {
          _messages[lang] = result.data;
        } else {
          console.error(`xnoteI18n: background failed to load ${lang} messages`, result?.error);
          _messages[lang] = {};
        }
      } catch (e2) {
        console.error(`xnoteI18n: failed to load ${lang} messages`, e2);
        _messages[lang] = {};
      }
    }
  }
   ...
   * Always falls back to chrome.i18n.getMessage() if custom lookup misses.
   */
  function t(key, substitutions) {
    ...
    // Not initialized OR custom lookup missed → always fallback
